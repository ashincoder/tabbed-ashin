...
void focus(int c) {
  ...
  if (nclients == 0) {
    ...
    xsettitle(win, buf);
    XChangeProperty(dpy, win, wmatom[WMIcon], XA_CARDINAL, 32, PropModeReplace,
                    (unsigned char *)icon, ICON_WIDTH * ICON_HEIGHT + 2);
  ...
  if (ev->state == PropertyNewValue && ev->atom == wmatom[WMSelectTab]) {
    ...
    if (c == sel)
      xseticon();
             ...
             (c = getclient(ev->window)) > -1) {
    updatetitle(c);
  } else if (ev->atom == wmatom[WMIcon] && (c = getclient(ev->window)) > -1 &&
             c == sel) {
    xseticon();
  }
}
...
void setup(void) {
  ...
  attrs.event_mask = FocusChangeMask | KeyPressMask | ExposureMask |
  ...
  for (int i = 0; i < ICON_WIDTH * ICON_HEIGHT; ++i) {
    icon[i + 2] =
        ICON_PIXEL_DATA[i * 4 + 3] << 24 | ICON_PIXEL_DATA[i * 4 + 0] << 0 |
        ICON_PIXEL_DATA[i * 4 + 1] << 8 | ICON_PIXEL_DATA[i * 4 + 2] << 16;
  }
...
void xseticon(void) {
  ...
  if (XGetWindowProperty(dpy, clients[sel]->win, wmatom[WMIcon], offset,
                         LONG_MAX, False, XA_CARDINAL, &ret_type, &ret_format,
                         &ret_nitems, &ret_nleft, &data) == Success &&
      ret_type == XA_CARDINAL && ret_format == 32) {
    XChangeProperty(dpy, win, wmatom[WMIcon], XA_CARDINAL, 32, PropModeReplace,
                    data, ret_nitems);
  } else if ((cwmh = XGetWMHints(dpy, clients[sel]->win)) &&
             cwmh->flags & IconPixmapHint) {
